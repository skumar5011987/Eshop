...
def get_file_path(request, filename):
    original_filname = filename
    nowTime = datetime.datetime.now().strftime('%y%m%d%H:%M:%S')
    filename = "%s%s" % (nowTime, original_filname)
...
class Order(models.Model):
    ...
    mobile = models.CharField(max_length=150, null=False, blank=False)
    ...
    status = models.CharField(
        max_length=150, choices=orderstatus, default="Pending")
    ...
    def __str__(self):
        return '{}_{}'.format(self.id, self.tracking_no)
...
class OrderItems(models.Model):
    ...
    def __str__(self):
        return '{}_{}'.format(self.id, self.order)
